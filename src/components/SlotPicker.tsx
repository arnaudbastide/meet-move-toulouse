...
const SlotPicker = ({ slots, onSlotSelect, maxPlaces = 0, disabled = false }: SlotPickerProps) => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Crenaux disponibles</CardTitle>
        ...
        <div className="flex flex-col gap-3">
          {slots.length === 0 ? (
            ...
            slots.map((slot) => {
              ...
              const available = maxPlaces > 0 ? maxPlaces - slot.booked_places : 0;
                ...
                <Card key={slot.id} className={full ? 'opacity-50' : ''}>
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="font-semibold">{formatDateTime(slot.start_at)}</p>
                      ...
                      {maxPlaces > 0 && (
                        <div className="text-right text-sm">
                          <p className={full ? 'text-destructive font-semibold' : 'text-muted-foreground'}>
                            {full ? 'Complet' : `${available} place${available > 1 ? 's' : ''} disponible${available > 1 ? 's' : ''}`}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {slot.booked_places} / {maxPlaces} réservé{slot.booked_places > 1 ? 's' : ''}
                          </p>
                        </div>
                      )}
                    </div>
